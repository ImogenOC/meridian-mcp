...
| `dm_list_types` | Enumerate parsed type paths, optionally restricted by a type-path prefix and maximum traversal depth. Results are paginated: the default page is 100 types, the maximum is 500, and `pagination.next_cursor` continues the same filtered traversal. |
| `dm_search_symbols` | Find type, procedure, variable, and macro names by case-insensitive partial match, with an optional symbol-kind filter. The default result limit is 50 and the maximum is 200. |
...
Active operations are available only in development mode. `dm_compile` invokes DreamMaker directly. For Meridian-Rift, `rift_compile` invokes the separate agent-owned `RIFT_BUILD.cmd`; humans continue to use the authoritative `BUILD.cmd`. The MCP passes validated inner wall/idle limits to RIFT, reserves up to 60 seconds of its outer wall limit for RIFT cleanup and reporting, and does not apply the child idle deadline to the normally silent wrapper. New RIFT controllers return one versioned `RIFT_RESULT` line whose artifact hashes and reuse state are checked against the canonical post-build snapshots; the exact legacy DM cache marker remains accepted for older compatible branches. Full-build output can optionally include bounded, observational endpoint samples, but `capture_complete` is always `false`.
