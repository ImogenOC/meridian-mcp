...
foreach ($trace in $traces) {
	...
	if (-not $sidecar.capture.validation.valid -or $sidecar.capture.validation.complete_frames -lt 3 -or $sidecar.capture.validation.trace_end_ns -le $sidecar.capture.validation.trace_begin_ns -or $sidecar.capture.validation.queue.dropped_events -ne 0) { throw "Capture validation failed: $($trace.Name)" }
	if ($sidecar.capture.validation.queue.saturation_count -gt 0) {
		$warnings += [ordered]@{ trace = $trace.Name; code = 'queue_saturation_observed_without_data_loss'; saturation_count = $sidecar.capture.validation.queue.saturation_count }
	}
	...
	if ($roles -notcontains 'dream_daemon' -or $roles -notcontains 'collector') { throw "Role-specific memory series are incomplete: $($trace.Name)" }
...
[pscustomobject]@{ schema = 2; status = 'passed'; experiment_id = $experimentId; meridian_mcp_build_id = $buildId; phase = $phase; controls = $validated; warnings = $warnings } | ConvertTo-Json -Depth 5
